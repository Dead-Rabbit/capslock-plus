...
transApiInit:
global deeplApiString:=""

global youdaoApiKey0, youdaoApiKey1
youdaoApiKey0=04f15c70-df90-c1bf-0250-f43cd232b2fe:fx
...
if(CLSets.TTranslate.apiType=1)
{
	if(CLSets.TTranslate.apiKey!="")
	{
		...
		deeplApiString=http://fanyi.youdao.com/paidapi/fanyiapi?key=%key%&type=data&doctype=json&q=
	}
	else if(youdaoApiKey1)
	{
		deeplApiString=http://fanyi.youdao.com/paidapi/fanyiapi?key=%youdaoApiKey1%&type=data&doctype=json&q=
	}
}
else
{
	...
	{
		key:=CLSets.TTranslate.apiKey
		deeplApiString=https://api-free.deepl.com/v2/translate?auth_key=%key%&target_lang=ZH&text=
	}
	else if(youdaoApiKey0)
	{
		deeplApiString=https://api-free.deepl.com/v2/translate?auth_key=%youdaoApiKey0%&target_lang=ZH&text=
	}
}
return
...
else ;IfWinNotExist,  ahk_id %transGuiHwnd% ;DeepL 翻译
{
	;~ MsgBox, 0
...
sendStr:=deeplApiString . UTF8Codes
...
; MsgBox, % responseStr
; MsgBox, % JSON.stringify(transJson) ;弹出整个翻译结果的json，测试用
...
;================拼MsgBox显示的内容
{
	Loop
	{
		if (transJson.translations[A_Index])
		{
			if (%A_Index%>1)
			{
				MsgBoxStr:=% MsgBoxStr . A_Space . ";" . A_Space  ;给每个结果之间插入" ; "
			}
			MsgBoxStr:= % MsgBoxStr . transJson.translations[A_Index].text	;翻译结果
		}
		else
		{
